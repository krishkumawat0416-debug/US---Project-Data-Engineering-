...
**~6–8M rows · ~1–2GB raw data · 6 months (2021–2022) · Medallion Architecture**
...
| **Dataset Size** | ~1–2 GB raw CSV · ~6–8M rows across 6 months |
| **Date Range** | 2021 – 2022 (3 months each year) |
...
> Only 3 months per year were processed to optimize runtime and storage during development/testing.
...
## ⚠️ Real-World Challenges Faced

| **Issue** | **Resolution** |
|---|---|
| Snowflake IAM ARN authorization failed during Storage Integration setup | Updated AWS IAM trust relationship using Snowflake-generated External ID and IAM user ARN |
| Databricks Serverless did not support direct Spark → Snowflake write | Exported Gold tables as Parquet to S3 and used Snowpipe for ingestion |
| Column mismatch caused NULL values in Snowflake | Used `MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE` and aligned column names |
| Snowpipe status was `RUNNING` but data was not loading | Triggered manual refresh using `ALTER PIPE pipe_name REFRESH;` |
| BTS CSV files contained trailing commas and corrupt records | Used explicit schema + `PERMISSIVE` mode with `_corrupt_record` column |
| Community Edition cluster auto-terminated during long jobs | Processed data in smaller yearly/monthly batches |
...
<details>
<summary><b>Snowflake AWS ARN / IAM Role Authorization Issue</b></summary>

- Storage Integration initially failed due to incorrect AWS IAM trust relationship configuration
- Retrieved Snowflake-generated IAM User ARN and External ID using:
  ```sql
  DESC INTEGRATION s3_integration;
  ```
- Updated AWS IAM Role Trust Policy and verified access using:
  ```sql
  LIST @gold_stage_s3;
  ```
...
<details>
<summary><b>Databricks Serverless Direct Snowflake Write Unsupported</b></summary>
...
- Direct Spark write:
  ...
  failed with:
  ```
  UNSUPPORTED_DATA_SOURCE_WRITE
  ```
- Solution:
  - Export Gold tables as Parquet to S3
  - Load into Snowflake using External Stage + Snowpipe
