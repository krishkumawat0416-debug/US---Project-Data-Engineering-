...
This project builds an end-to-end **Data Lakehouse pipeline** for analyzing U.S. domestic airline on-time performance. Raw CSV data from the Bureau of Transportation Statistics (BTS) flows through a **Medallion Architecture** (Bronze → Silver → Gold) in Databricks using Apache Spark and Delta Lake, before being served via Snowflake to a Power BI dashboard.
...
| **Processing** | Apache Spark on Databricks Community Edition |
| **Storage** | Amazon S3 + Delta Lake |
| **Serving** | Snowflake (free trial) |
| **Reporting** | Power BI Desktop (free) |
| **Total Cost** | ~$5–$20 (mostly free/trial tier) |
...
- Skips months already downloaded (`os.path.exists` check)
...
**Key behaviours:**
- Parses `year` and `month` directly from the BTS filename (split on `_`)
...
> BTS CSVs end every row with a trailing comma, creating an unnamed 111th column. `inferSchema` is also slow on 7 GB of data. An explicit schema is always used in production.
...
> **Note:** `Flight_Number_Reporting_Airline` is intentionally defined as `IntegerType` here — this is flagged in the code as a mistake to be corrected in Silver (it should be `StringType` since leading zeros matter).
...
`PERMISSIVE` mode captures bad rows in `_corrupt_record` instead of crashing the entire job.
...
Partitioning by `year` / `month` enables Spark to skip irrelevant partitions and dramatically reduces query time.
...
Silver is the **transformation and enrichment layer**. The Bronze Delta table is loaded and progressively enhanced across 56 cells.
...
`broadcast()` hint sends the small airport reference to all worker nodes — avoids shuffling 7 GB of flight data.
...
Used for cross-validating BTS scheduled distances — flags routes where BTS distance deviates >15% from the computed great-circle distance.
...
`dataSkippingNumIndexedCols=5` indexes the first 5 columns for data skipping — speeds up filtered reads by 60–80%.
...
**Steps 5–8 — Create 4 Staging Tables** (`monthly_airline_kpi`, `annual_route_performance`, `gold_airport_departure_kpi`, `delay_cause_table`)

**Step 9 — Auto-Ingest Pipes:**
...
Same pattern for the other 3 pipes. `AUTO_INGEST = TRUE` means Snowflake automatically triggers ingestion when new Parquet files appear in S3 (via SQS events).
...
### Bronze — Observation Only (additive `dq_*` flags)
...
| DQ flag columns | 7 additive `dq_*` columns on Bronze (Phase 2) |
